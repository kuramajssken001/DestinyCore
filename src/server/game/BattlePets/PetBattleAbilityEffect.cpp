...
int32 PetBattleAbilityEffect::GetPetType()
{
    if (BattlePetAbilityTurnEntry const* abilityTurn = sBattlePetAbilityTurnStore.LookupEntry(EffectInfo->BattlePetAbilityTurnID))
        if (BattlePetAbilityEntry const* ability = sBattlePetAbilityStore.LookupEntry(abilityTurn->BattlePetAbilityID))
            return ability->PetTypeEnum == BATTLEPET_PETTYPE_ALL ? BATTLEPET_PETTYPE_HUMANOID : ability->PetTypeEnum;
...
bool PetBattleAbilityEffect::HandleHealPercentDealt()
{
    ...
    // Recovery
    int32 heal = CalculatePct(GetState(Caster, BATTLEPET_STATE_Last_HitDealt), EffectInfo->Param[0]);
    int32 modPct = 0;
...
{
    int32 chance = EffectInfo->Param[0] + (GetState(Target, BATTLEPET_STATE_Internal_CaptureBoost) * EffectInfo->Param[1]);
...
bool PetBattleAbilityEffect::HandleHealLastHitTaken()
{
    ...
    // Recovery
    int32 heal = CalculatePct(GetState(Caster, BATTLEPET_STATE_Last_HitTaken), EffectInfo->Param[0]);
    int32 modPct = 0;
...
{
    assert(EffectInfo->Param[0] < NUM_BATTLEPET_STATES);
...
bool PetBattleAbilityEffect::HandleHealPetType()
{
    ...
    for (const auto& currentPet : PetBattleInstance->Pets)
    {
        if (currentPet && currentPet->TeamID == team)
        {
            ...
            {
                auto heal = (EffectInfo->Param[0] / 2) + CalculatePct((EffectInfo->Param[0] / 2), modPct + GetState(currentPet->ID, BATTLEPET_STATE_Mod_HealingTakenPercent));
...
bool PetBattleAbilityEffect::HandleDamagePercentTaken()
{
    ...
    int32 damage = CalculateDamage(CalculatePct(GetState(Caster, BATTLEPET_STATE_Last_HitTaken), EffectInfo->Param[0]));
    return Damage(Target, damage);
}
